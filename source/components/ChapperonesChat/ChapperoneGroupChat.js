...
import {getInputProp, checkIfUserAlreadyRead} from './helpers';
...
class BroadcastChats extends Component {
  ...
  async componentDidMount() {
    ...
    if (noContractOrExpiredContract(this.user)) {
      Actions.pop();
      return;
    }
  ...
  updateReadReceipts = async (tripId, user, messages) => {
    const updates = {};
  ...
  navigateToRead = messageId => {
    Actions.ChapperoneReadList({tripId: this.props.trip._id, messageId});
  };
