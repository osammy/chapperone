...
class BroadcastChats extends Component {
    ...
    RnBgTask.runInBackground(async () => {
      try {
        ...
        if (messages[0]) {
          ...
          const key = tripId;
          const dataToStore = {};
          const msgs = this.getMessagesNotInState(plainArrMessages);
          dataToStore[key] = msgs;

          this.props.setChapperonesMessages(dataToStore);
          // Get the messages whose status isnt received
          const unreadMsgs = plainArrMessages.filter(
          ...
          if (unreadMsgs.length > 0) {
            ...
            await this.updateReadReceipts(tripId, user, unreadMsgs);
  ...
  updateReadReceipts = (tripId, user, messages) => {
    ...
    messages.forEach(async message => {
      ...
      if (shouldBeUpdated) {
        ...
        updates[
          `${chapperones_read_receipts_node}/${read_receipt_key}`
        ] = payload;
      }
      if (count === msgLength) {
        return firebase
  ...
  navigateToRead = messageId => {
    Actions.ChapperoneSeenList({tripId: this.props.trip._id, messageId});
  };
