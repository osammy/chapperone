...
function ChapperoneReadReceipts(props) {
  ...
  useEffect(() => {
    ...
    const ref = firebase
      .database()
      .ref(`/chapperones_read_receipts/${props.tripId}/${props.messageId}`);
    ...
    const handler = async snapShot => {
      ...
      props.participants.forEach(eachParticipant => {
        const userId = eachParticipant?.user_id?._id;
        const isReadUser = seenDict[userId];
        if (isReadUser) {
            ...
            <View style={styles.subHeaderContainer}>
              <Block row space="between">
                ...
                <Text style={styles.countText} gray semibold>{`(${
                  unReadUsers.length
                } / ${props.participants?.length - 1})`}</Text>
              </Block>
            </View>
...
export default connect(mapStateToProps)(ChapperoneReadReceipts);
