...
function BroadcastGroup(props) {
  const {tripId, lastBroadcastMessage, handleBroadcast} = props;
  let name = '';
  let data;
  const key = tripId;

  try {
    data = lastBroadcastMessage[key];
    name = `${data.name}: `;
  } catch (e) {
    data = {};
  }
  ...
  return (
    <TouchableHighlight style={styles.container} onPress={handleBroadcast}>
      <View style={styles.listItem}>
        ...
        <View
          style={[
            styles.adminContent,
            // {backgroundColor: 'black', borderColor: 'red', height: 60},
          ]}>
          ...
          <View style={styles.spaceBetween}>{renderContent()}</View>
        </View>
      </View>
    </TouchableHighlight>
  );
